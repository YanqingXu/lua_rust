...
也已迁移，但唯一 Heap owner、字符串 scoped/canonical Eq/Hash、真实 GC 回收、write
...
| GC object/header/ref | `crates/lua_core/src/gc/header.rs`、`gc_ref.rs`、`gc_object.rs` | ObjectId/live-table provenance、foreign/stale/type rejection、result-slice exact-id root 与 GC 模块测试 | partial：唯一 Heap owner、字符串/执行 scoped borrow 尚未完成 |
| Table/metatable | `table.rs`、`metatable.rs` | table/value 项目内测试和 Lua fixtures | partial：write barrier、GC 周期和 oracle 边角未验证 |
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler/library/IO/VM publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function、library/package Function、IO iterator graph、VM closure/open-Upvalue 与 synchronous result 事务发布、跨 state GET/SET、root fixed-point 与 close-order 回归 | partial：debug/protected-helper 跨 state、barrier、string borrow 与 live sweep 未闭环 |
| Userdata/thread/state | `userdata.rs`、`thread.rs`、`io.rs`、`lua_vm::runtime` | IO/proxy Userdata graph、StateHandle identity/retirement、PendingState rollback/root seed、coroutine create/wrap、CLI argument explicit-root handoff、trampoline、typed DropProbe 与 1000 轮 shutdown | partial：finalizer/service close、main/Heap owner 与 scoped string access 未闭环 |
...
- 当前 771 项 workspace tests、24/24 inventory 和 shutdown/owner 回归不能
