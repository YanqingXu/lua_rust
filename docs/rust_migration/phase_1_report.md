...
trampoline、`TEMPORARY_STATE_ROOTS/PendingState`、canonical mark-only tracer
与确定性 shutdown substrate 已有本地实现和回归；但唯一 Heap owner、其余生产
...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、Runtime root/transfer 路径 | managed Proto、checked owner、跨 state GET/SET、root fixed-point 与 close-order 回归 | partial：debug/protected-helper 跨 state、barrier 与 live sweep 未闭环 |
...
| Runtime/Heap 与 publication 所有权未闭环 | `LuaState` 仍保存 transitional service backpointer，main state 为 external arena slot；temporary state/coroutine publication 已迁移，但 Proto/compiler/library/IO/VM/app 等对象图仍未迁移 | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.4–M1.8 |
...
- 当前 750 项 workspace tests、24/24 inventory 和 shutdown/owner 回归不能
