...
StateArena/StateHandle、checked open-Upvalue owner、Runtime coroutine 与
arbitrary-Lua `pcall/xpcall` activation trampoline、debug/protected
Runtime-native upvalue scheduling、
...
shutdown fault matrix、128 层 nested protected call 也已形成本地回归；
其他同步 callback helper suspension、通用非字符串 scoped access 与显式
service drain 尚未闭环。
...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler/library/IO/VM publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function、library/package Function、IO iterator graph、VM closure/open-Upvalue 与 synchronous result 事务发布、跨 state GET/SET、debug direct/`pcall` Runtime-native owner turn、arbitrary-Lua `pcall/xpcall` target/handler suspension、1000 层 resume/wrap、128 层 nested protected call、三层 Normal+owner turn、mailbox/owner/response/unwind/shutdown fault matrix、foreign/stale error response、root fixed-point、STW/incremental prepass 与 close-order回归 | partial：sealed debug get/setup、`pcall/xpcall`、深链与已命名 fault boundaries 已闭合；其他同步 callback helper suspension 与通用非字符串 scoped access 仍未闭环 |
...
| 阻塞 | 现场证据 | 跟踪 |
|---|---|---|
| 其他同步 callback helper suspension 尚未闭环 | arbitrary-Lua `pcall/xpcall` 已由 Runtime activation 驱动；metamethod/load reader/table comparator/debug hook 等同步 `call_value_with_results` 路径遇 Runtime-native request 或 ordinary cross-state Upvalue suspension 仍 fail-closed | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.5 |
...
- 当前 Debug/Release 各 838 项 workspace tests、24/24 root inventory、
  ...
  回归不能替代通用 non-string scoped access、其他同步 callback suspension
  与跨平台 sanitizer 证据。
