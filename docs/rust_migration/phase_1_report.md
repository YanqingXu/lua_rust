...
实现和回归；library registration 与 package graph publication 也已迁移，但
唯一 Heap owner、IO/VM/app/results 生产 publication、真实 GC 回收、write
...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler/library publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function、library C/runtime-native Function 与 package graph 事务发布、checked owner、跨 state GET/SET、root fixed-point 与 close-order 回归 | partial：IO/VM/app/results publication、debug/protected-helper 跨 state、barrier 与 live sweep 未闭环 |
...
| Runtime/Heap 与 publication 所有权未闭环 | `LuaState` 仍保存 transitional service backpointer，main state 为 external arena slot；temporary state/coroutine、compiler Proto→Function、library registration 与 package graph publication 已迁移，但 IO/VM/app/results 等对象图仍未迁移 | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.4–M1.8 |
...
- 当前 758 项 workspace tests、24/24 inventory 和 shutdown/owner 回归不能
