...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler/library/IO/VM publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function、library/package Function、IO iterator graph、VM closure/open-Upvalue 与 synchronous result 事务发布、跨 state GET/SET、root fixed-point、STW/incremental prepass 与 close-order 回归 | partial：Lua-visible full/explicit incremental collection 已在 released state-turn safe point 工作；debug/protected-helper 跨 state 与 automatic sweep 未闭环 |
...
- 当前 Debug/Release 各 815 项 workspace tests、24/24 root inventory、
  8/8 mutation inventory、54-path heap contract、17-path string contract
  和 shutdown/owner 回归不能替代 automatic checkpoint、allocator live
  bytes 和 sanitizer/Miri 证据。
