...
owner 也已迁移。crate-private Runtime-only strong-graph STW 已消费 canonical
tracer、预清理不可达 state 并执行真实 sweep/accounting；但 weak/finalizer、
Lua-visible GC、write barrier 和完整 shutdown 尚未闭环。
因此，本阶段不能标记 completed。

| 分类 | 本阶段内容 |
|---|---|
| `completed` | 无。尚没有满足双 oracle 与阶段完成门槛的整项能力。 |
| `partial` | Value/GC object、table/metatable、function/proto/upvalue、userdata/thread、Runtime/StateArena、canonical roots、Runtime-only strong-graph STW、shutdown 与 weak/finalize/sweep 组件。 |
| `not-started` | Lua-visible/automatic/incremental collection、完整 barrier/finalizer/resurrection、allocator soak 与 Lua C API owner 合同。 |
...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler/library/IO/VM publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function、library/package Function、IO iterator graph、VM closure/open-Upvalue 与 synchronous result 事务发布、跨 state GET/SET、root fixed-point、STW prepass 与 close-order 回归 | partial：内部 STW 已在零 active execution 时工作；debug/protected-helper 跨 state、barrier 与 public/incremental sweep 未闭环 |
| Userdata/thread/state | `userdata.rs`、`thread.rs`、`io.rs`、`lua_vm::runtime` | IO/proxy Userdata graph、StateHandle identity/retirement、PendingState rollback/root seed、coroutine create/wrap、CLI argument handoff、trampoline、scoped services、unreachable-state prepass、typed DropProbe 与 1000 轮 shutdown | partial：唯一 Heap/service owner 与 internal state reclaim 已完成；finalizer/service close 与 main-state arena slot 统一仍未闭环 |
| Mark/weak/finalize/sweep 组件 | `gc/mark.rs`、`weak.rs`、`finalize.rs`、`sweep.rs`、`runtime/full_collection.rs` | 两轮 strong-graph sweep、reachable/unreachable object/state、Upvalue close-before-handle-invalid、typed Drop、cross-collector/root-gap/weak/finalizer/phase gates | partial：Runtime-only strong-graph STW 已执行真实 sweep；weak/finalizer/resurrection 与 stdlib GC 控制路径仍未接入 |
...
| 阻塞 | 现场证据 | 跟踪 |
|---|---|---|
| `collectgarbage` 没有运行完整 sweep，计数和 step 为模拟值 | crate-private `Runtime::collect_full_stw` 已真实回收强图；`base.rs` 的 `poll_gcinfo_kb`/`step_gcinfo_cycle` 尚未调用它，且 weak/finalizer 被显式拒绝 | [NOTE-002](deviation_log.md#note-002-gc-可观察行为尚未形成真实回收闭环)，M1.9–M1.13 |
| 写屏障函数存在但 mutation site 仍有 TODO | table/function/upvalue/userdata/thread 的 setter 注释 | NOTE-002，M1.11 |
| Lua-visible all-graph collection 尚未闭环 | Runtime-only STW 已消费 canonical tracer、拒绝所有 gap/foreign edge、预清理不可达 state 并 sweep；active weak tables、pending/new finalizers、public/automatic/incremental 入口仍 fail-closed | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.9–M1.12 |
...
- 当前 790 项 workspace tests、24/24 root inventory、53-path heap contract、
  17-path string contract
  和 shutdown/owner 回归不能
  替代 Lua-visible weak/finalizer/resurrection、allocator live bytes 和
  sanitizer/Miri 证据。
