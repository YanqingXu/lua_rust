...
trampoline、canonical mark-only tracer 与确定性 shutdown substrate 已有本地
实现和回归；但唯一 Heap owner、生产 publication、真实 GC 回收、write barrier
和完整 Lua-visible shutdown 尚未闭环。因此，本阶段不能标记 completed。
...
| Userdata/thread/state | `userdata.rs`、`thread.rs`、`lua_vm::runtime` | StateHandle identity/retirement、coroutine trampoline、typed DropProbe 与 1000 轮 shutdown | partial：finalizer/service close、main owner、temporary state roots 未闭环 |
...
| Runtime/Heap 与 publication 所有权未闭环 | `LuaState` 仍保存 transitional service backpointer，main state 为 external arena slot，temporary state/production publication roots 未迁移 | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.4–M1.8 |
...
- 当前 745 项 workspace tests、24/24 inventory 和 shutdown/owner 回归不能
