...
实现和回归；但唯一 Heap owner、library/package/IO/VM/app/results 生产
publication、真实 GC 回收、write barrier 和完整 Lua-visible shutdown 尚未
闭环。因此，本阶段不能标记 completed。
...
| Function/Proto/upvalue | `function.rs`、`proto.rs`、`upvalue.rs`、compiler publication、Runtime root/transfer 路径 | managed Proto、compiler string/child-Proto/top-Function 事务发布、checked owner、跨 state GET/SET、root fixed-point 与 close-order 回归 | partial：其余 library/IO/VM/app publication、debug/protected-helper 跨 state、barrier 与 live sweep 未闭环 |
| Userdata/thread/state | `userdata.rs`、`thread.rs`、`lua_vm::runtime` | StateHandle identity/retirement、PendingState exact-id rollback/root seed、coroutine create/wrap transactional publication、trampoline、typed DropProbe 与 1000 轮 shutdown | partial：finalizer/service close、main owner 与其余生产 publication 未闭环 |
...
| Runtime/Heap 与 publication 所有权未闭环 | `LuaState` 仍保存 transitional service backpointer，main state 为 external arena slot；temporary state/coroutine 与 compiler Proto→Function publication 已迁移，但 library/package/IO/VM/app/results 等对象图仍未迁移 | [NOTE-009](deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环)，M1.4–M1.8 |
...
- 当前 754 项 workspace tests、24/24 inventory 和 shutdown/owner 回归不能
