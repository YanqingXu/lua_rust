...
| GC 引用安全 | `GcRef` 已携带进程级不复用 `ObjectId`，collector live table 在解引用前校验地址、身份与类型；managed Proto 与临时对象根基础已落地。`Value::String` 内容 Eq/Hash、open-Upvalue raw owner、临时状态根和未迁移 publication 路径仍阻止 live sweep | P0 |
