...
| GC 与 runtime lifecycle | Runtime shutdown、collector provenance、StateHandle identity/retirement、checked open-Upvalue owner、`PendingState` 临时状态根、coroutine activation、production mutation barrier、weak/finalizer/resurrection、Lua-visible full STW 与五阶段 incremental step 已完成；count 使用 collector accounted bytes。main-state owner、debug/protected-helper 跨 state、allocation-triggered automatic GC、allocator live/peak 和显式 service drain 未闭环。 | [NOTE-002](docs/rust_migration/deviation_log.md#note-002-gc-增量与-allocator-可观察行为尚未闭环)、[NOTE-009](docs/rust_migration/deviation_log.md#note-009-runtime-与-coroutine-所有权未闭环) |
